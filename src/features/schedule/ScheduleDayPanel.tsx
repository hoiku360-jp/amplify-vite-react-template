...
export default function ScheduleDayPanel(props: { owner: string }) {
  ...
  async function saveParentNotice(textOverride?: string) {
    ...
    try {
      ...
      const updateRes = await client.models.ScheduleDay.update({
        ...
        // 本文を確定保存し直した場合、過去の共有・送信状態はリセットする
  ...
  async function updateParentNoticeDeliveryState(input: {
    ...
    const updateRes = await client.models.ScheduleDay.update({
      ...
      parentNoticeConfirmedAt: day.parentNoticeConfirmedAt || now,
      parentNoticeDeliveryMethod: input.deliveryMethod,
      parentNoticeSharedAt: input.sharedAt ?? day.parentNoticeSharedAt ?? null,
      parentNoticeSentAt: input.sentAt ?? day.parentNoticeSentAt ?? null,
  ...
  async function shareParentNotice() {
    ...
    try {
      const shareData: NativeShareData = {
        title: buildParentNoticeShareTitle(day),
        text,
      };
  ...
  async function markParentNoticeSent() {
    ...
    try {
      const now = new Date().toISOString();
      const deliveryMethod = s(day.parentNoticeDeliveryMethod) || "MANUAL";
      ...
      await updateParentNoticeDeliveryState({
        ...
        deliveryMethod:
          deliveryMethod === "WEB_SHARE" || deliveryMethod === "CLIPBOARD"
            ? deliveryMethod
            : "MANUAL",
        sharedAt: day.parentNoticeSharedAt ?? now,
        sentAt: now,
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      {day ? (
        ...
        >
          {" "}
          <h3 style={{ margin: 0 }}>保護者向け連絡候補</h3>
          <div style={{ fontSize: 13, color: "#555" }}>
            今日の予定・家庭への連絡事項・Practiceをもとに、保護者向けのお知らせ下書きを生成します。
            保育士が確認・編集して確定したうえで、iPhoneの共有機能またはコピーで実際の連絡アプリへ渡します。
          </div>
          <label>
          ...
          <div style={{ fontSize: 12, color: "#666", lineHeight: 1.7 }}>
            status: {parentNoticeStatusLabel(day.parentNoticeStatus)} / method:{" "}
            {parentNoticeDeliveryMethodLabel(day.parentNoticeDeliveryMethod)}
            <br />
            generatedAt: {formatDateTime(day.parentNoticeGeneratedAt)} /
            confirmedAt: {formatDateTime(day.parentNoticeConfirmedAt)}
            <br />
            sharedAt: {formatDateTime(day.parentNoticeSharedAt)} / sentAt:{" "}
            {formatDateTime(day.parentNoticeSentAt)}
          </div>
        </div>
      ) : null}
